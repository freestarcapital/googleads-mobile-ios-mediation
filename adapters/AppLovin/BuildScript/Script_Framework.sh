...
/bin/ln -sfh "Versions/Current/${FRAMEWORK_NAME}" "${FRAMEWORK_LOCATION}/${FRAMEWORK_NAME}"

# Build static library for iOS Device.
xcodebuild -target AppLovinAdapter ONLY_ACTIVE_ARCH=NO -configuration "${CONFIGURATION}" clean build -sdk "iphoneos" ARCHS="armv7 arm64" BUILD_DIR="${BUILD_DIR}" BUILD_ROOT="${BUILD_ROOT}" OBJROOT="${OBJROOT}" SYMROOT="${SYMROOT}" "${ACTION}" -UseModernBuildSystem=NO

# Build static library for iOS Simulator.
xcodebuild -target AppLovinAdapter ONLY_ACTIVE_ARCH=NO -configuration "${CONFIGURATION}" clean build -sdk "iphonesimulator" ARCHS="i386 x86_64" BUILD_DIR="${BUILD_DIR}" BUILD_ROOT="${BUILD_ROOT}" OBJROOT="${OBJROOT}" SYMROOT="${SYMROOT}" "${ACTION}" -UseModernBuildSystem=NO
...
echo "${SRCROOT}"
echo "${OUTPUT_FOLDER}/${FRAMEWORK_NAME}.framework/Versions/A/Headers"
/bin/cp -a "${SRCROOT}/Public/Headers/" "${OUTPUT_FOLDER}/${FRAMEWORK_NAME}.framework/Versions/A/Headers"
